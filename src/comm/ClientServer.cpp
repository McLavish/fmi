...
void FMI::Comm::ClientServer::send(channel_data buf, FMI::Utils::peer_num dest) {
    ...
    std::string file_name = object_key_prefix() + std::to_string(peer_id) + "_" + std::to_string(dest) + "_" + std::to_string(operation_num);
    operation_num++;
...
void FMI::Comm::ClientServer::recv(channel_data buf, FMI::Utils::peer_num dest) {
    ...
    std::string file_name = object_key_prefix() + std::to_string(dest) + "_" + std::to_string(peer_id) + "_" + std::to_string(operation_num);
    operation_num++;
...
void FMI::Comm::ClientServer::bcast(channel_data buf, FMI::Utils::peer_num root) {
    std::string file_name = object_key_prefix() + std::to_string(root) + "_bcast_" + std::to_string(num_operations["bcast"]);
...
void FMI::Comm::ClientServer::barrier() {
    ...
    std::string file_name = object_key_prefix() + std::to_string(peer_id) + barrier_suffix;
...
void FMI::Comm::ClientServer::reduce(channel_data sendbuf, channel_data recvbuf, FMI::Utils::peer_num root, raw_function f) {
    ...
    if (peer_id == root) {
        ...
        std::string file_name = object_key_prefix() + std::to_string(peer_id) + "_reduce_" + std::to_string(operation_num);
...
void FMI::Comm::ClientServer::scan(channel_data sendbuf, channel_data recvbuf, raw_function f) {
    ...
    if (peer_id != num_peers - 1) {
        std::string file_name = object_key_prefix() + std::to_string(peer_id) + "_scan_" + std::to_string(operation_num);
        upload(sendbuf, file_name);
    }
    bool left_to_right = !(f.commutative && f.associative);
    ...
    while (elapsed_time < max_timeout && std::any_of(applied.begin(), applied.end(), [] (bool v) { return !v; }) ) {
        ...
        for (Utils::peer_num i = 0; i < num_data; i++) {
            ...
            std::string file_name = object_key_prefix() + std::to_string(i) + "_scan_" + std::to_string(operation_num);
